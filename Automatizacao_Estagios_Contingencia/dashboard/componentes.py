...
def gauge_estagio(classificacao: dict, tema: str = "dark") -> go.Figure:
    ...
    fig = go.Figure(go.Indicator(
        ...
        title={"text": f"<b>{estagio}</b>",
               "font": {"size": 26, "color": classificacao.get("cor", "#2E9E44")}},
    ))
...
def banner_estagio(classificacao: dict, timestamp: str) -> dbc.Alert:
    ...
    return dbc.Alert(
        [
            ...
            html.Ul([html.Li(j) for j in classificacao.get("justificativas", [])],
                    className="mb-0"),
        ],
...
def grafico_afluentes(series: dict, tema: str = "dark") -> go.Figure:
    ...
    cores = {"Rio_Gravatai": "#F2B90B", "Rio_dos_Sinos_SaoLeopoldo": "#5FD068",
             "Rio_Cai": "#F2830B", "Rio_Jacui_TriunfoAmarop": "#9B8CE0"}
    rotulos = {"Rio_Gravatai": "Gravataí", "Rio_dos_Sinos_SaoLeopoldo": "Sinos",
               "Rio_Cai": "Caí", "Rio_Jacui_TriunfoAmarop": "Jacuí"}
    fig = go.Figure()
...
def cards_rios(indicadores: dict, tema: str = "dark") -> dbc.Row:
    ...
    for info in config.INFO_RIOS_CARDS:
        ...
        if cota is None and info["chave"] == "Guaiba_PortoAlegre_CaisMaua":
            cota = config.COTA_INUNDACAO_GUAIBA
        ...
        if nivel is not None and cota:
            ...
            barra = html.Div()
            rodape = html.Small("cota de inundação a definir"
