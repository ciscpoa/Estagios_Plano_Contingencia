...
app.layout = dbc.Container(id="raiz", fluid=False,
    ...
    dbc.Row([
        ...
        dbc.Col(dbc.Card(dbc.CardBody([
            html.H6("Gatilhos de campo confirmados (Defesa Civil/SMS)",
                    className="text-light"),
            html.Small("Preenchidos via arquivo gatilhos_manuais.txt na pasta "
                       "do projeto (marque 'ok' no gatilho confirmado e "
                       "atualize). Gatilho confirmado eleva o estágio até a "
                       "coluna do Plano correspondente (regra de piso).",
    ...
    html.Div(id="area-arquivo", className="text-secondary small my-3 text-center"),
...
def renderizar(snapshot, tema):
    ...
    if not snapshot:
        ...
        return (aviso, html.Div(), html.Div(), html.Div(), vazio,
    ...
    if ativos:
        gat = html.Ul([html.Li(a, className="text-warning small text-start")
                       for a in ativos],
                      className="d-inline-block mb-0")
    else:
        gat = html.Div("Nenhum gatilho confirmado em campo no momento.",
    ...
    g_chuva = _seguro("grafico_precipitacao",
                      ...
                      componentes.grafico_precipitacao([], [], tema))
    arq = snapshot.get("csv_exportado", "")
    nota = (f"Dados brutos exportados (CSV + Excel) em: {arq} · "
            f"última extração: {snapshot.get('timestamp', '—')}") if arq else ""
    return banner, cards, regioes, gat, gauge, g_guaiba, g_afl, g_chuva, nota
