...
def _tentar_api() -> dict | None:
    try:
        r = requests.get("https://apiprevmet3.inmet.gov.br/avisos/ativos", timeout=20)
        r.raise_for_status()
        avisos = r.json()
    except Exception as exc:
        print(f"[INMET] API de avisos indisponível ({exc}); usando Selenium.")
        return None
    ...
    return {"alertas": encontrados, "fonte": "api"}
