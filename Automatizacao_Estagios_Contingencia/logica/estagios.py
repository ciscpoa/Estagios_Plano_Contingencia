...
def _avaliar_regras(
    ...
    if b1:
        ...
        if ind.inmet_max_severidade:
            extras.append(f"aviso INMET {ind.inmet_max_severidade}")
        if reg["n_total"]:
            nomes = ", ".join(reg.get("regioes_todas", [])[:5])
            extras.append(f"{reg['n_total']} alerta(s) da Defesa Civil"
                          + (f" — regiões: {nomes}" if nomes else ""))
        motivos.append(
            f"Previsão de chuvas mais intensas ({ind.previsto_48h_mm:.0f} mm/48h"
    ...
    if cond_atencao:
        ...
        for nome_afl, dados_afl in ind.afluentes.items():
            ...
            if ref_a is not None and nv_a is not None and nv_a >= ref_a:
                quais.append(f"{nome_afl} {nv_a:.2f} m (≥ {ref_a:.2f})")
    ...
    if cond_atencao:
        ...
        for nome, dados in ind.afluentes.items():
            ...
            if ref is not None and nv is not None and nv >= ref:
                quase.append(f"{nome} em {nv:.2f} m (≥ atenção {ref:.2f} m)")
    if subindo:
...
def ler_gatilhos_txt(caminho=None) -> InputsInfraestrutura:
    ...
    ativos = gatilhos_ativos(infra)
    if ativos:
        print(f"[Gatilhos] {len(ativos)} gatilho(s) confirmado(s) no txt: "
              + "; ".join(r for _, r in ativos))
    return infra
