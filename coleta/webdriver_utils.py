...
def criar_driver(headless: bool | None = None) -> webdriver.Chrome:
    ...
    # ── 1) Container/Docker (Render): Chrome e chromedriver já instalados,
    #    informados por CHROME_BIN e CHROMEDRIVER_PATH (ver Dockerfile) ──
    import os as _os
    chrome_env = _os.environ.get("CHROME_BIN")
    driver_env = _os.environ.get("CHROMEDRIVER_PATH")
    if chrome_env and Path(chrome_env).exists():
        opts.binary_location = chrome_env
        # Container (Render): reduz o consumo de memória do Chromium
        ...
        # janela menor = menos memória (o mapa continua clicável)
        opts.arguments[:] = [a for a in opts.arguments
                             if not a.startswith("--window-size")]
        opts.add_argument("--window-size=1366,768")
        if _os.environ.get("SELENIUM_SEM_IMAGENS", "").lower() in ("1", "true"):
